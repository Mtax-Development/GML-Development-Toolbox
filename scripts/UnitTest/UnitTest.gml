...
/// @description		Constructs a testing environment used with invidual chunks of code to automate
///						assertion of their intended behavior and stability.
...
function UnitTest(_name) constructor
{
  ...
  #region [Methods]
   ...
   #region <Getters>
	...
	static getResults = function()
	{
		...
		repeat (_testStatus_number)
		{
			...
			var _string_testName = ((testNames[_i[0]] != undefined) ? (" " + string(testNames[_i[0]]))
																	: "");
				...
				if (_failures_length == 1)
				{
					...
					switch (_failure.type)
					{
						...
						case "Assert: Not equal":
							_string_detail = (string(_failure.functionReturn) + _mark_notEqual +
   ...
   #region <Execution>
	...
	/// @description		Add to this constructor information with the specified test name about
	///						pairs of specified values returned by a function matching their expected
	///						results and use it with the logging function.
	///						The test is treated as successful if all values match their result.
	static assert_equal = function()
	{
		...
		repeat ((argument_count - 1) div 2)
		{
			...
			if (logAssertion != undefined)
			{
				...
				var _string_logAssertion =
				(
					testNames[testID] + " [" + string((_pair + 1)) + "]" + ": {" +
					string(_status.functionReturn) + _string_assertionSuccess +
	...
	///						pairs of specified values returned by a function matching their expected
	///						results and use it with the logging function.
	///						The test is treated as successful if no value matches its result.
	...
	/// @description		Add to this constructor information with the specified test name about
	///						specified functions executed with their arguments and use it with the
	///						logging function.
	///						The test is treated as successful if no errors are thrown.
	static assert_executable = function(_name)
	{
		...
		repeat (ceil((argument_count - 1) / 2))
		{
			...
			{
				_functionArgument = ((_functionArgument == undefined) ? [] : [_functionArgument]);
			}
			
			var _error = undefined;
			var __execution = ((is_method(__executedFunction)) ? method_get_index(__executedFunction)
															   : __executedFunction);
			...
			if (logAssertion != undefined)
			{
				...
				(
					testNames[testID] + " [" + string((_pair + 1)) + "]" + ": {" +
					string(_status.executedFunction) + "(" + _string_functionArguments + ")" + ": " +
	...
	/// @argument			name {string}
	/// @description		Add to this constructor information with the specified test name to note
	///						that it cannot be executed to test it.
	///						This test is always treated as successful.
	...
	/// @argument			name {string}
	/// @description		Add to this constructor information with the specified test name to note
	///						that it has not been created yet.
	///						This test is always treated as successful.
